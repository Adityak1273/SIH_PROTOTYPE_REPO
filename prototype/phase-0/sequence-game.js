/* Sequence Memory — original implementation using the supplied reference only for interaction structure. */
(function(){
  function install(){
    ...
    function runSequenceRound(){
      ...
      const sequence=Array.from({length},()=>Math.floor(Math.random()*catalog.sequence.pads.length));
      c.sequence=sequence;
      c.sequenceInput=[];
      c.sequenceLocked=true;
      c.sequenceRoundStarted=performance.now();
      ...
      area.innerHTML=`<div class="sequence-stage sequence-transition">
        <div class="sequence-meta">
          ...
          <span class="sequence-chip">Level <strong>${level}</strong></span>
          <span class="sequence-chip">Score <strong>${c.sequenceScore}</strong></span>
          <span class="sequence-chip">Misses <strong>${c.sequenceConsecutiveMistakes} / 3</strong></span>
        </div>
        <div class="sequence-progress" aria-label="Game progress">
          <span style="width:${Math.round((c.sequenceRound-1)/c.sequenceMaxRound*100)}%"></span>
        </div>
        ...
        <div class="sequence-board" aria-label="Sequence memory board">
          ${catalog.sequence.pads.map((p,i)=>`<button class="sequence-pad" data-pad="${i}" data-color="${p.color}" type="button" aria-label="${p.label}" disabled>${p.icon}</button>`).join('')}
        </div>
        <div class="sequence-count" id="sequenceCount">Get ready…</div>
        <div class="sequence-feedback" id="sequenceFeedback" role="status" aria-live="polite"></div>
      </div>`;
      ...
      function handlePad(chosen){
        ...
        if(chosen===expected){
          ...
          if(c.sequenceInput.length===c.sequence.length){
            const responseMs=performance.now()-c.sequenceRoundStarted;
            c.sequenceScore+=20;
            c.sequenceStreak++;
            c.sequenceConsecutiveMistakes=0;
            c.sequenceRoundResults.push({round:c.sequenceRound,correct:true,length:c.sequence.length,responseMs});
        ...
        c.sequenceStreak=0;
        c.sequenceRoundResults.push({round:c.sequenceRound,correct:false,length:c.sequence.length,expected,chosen,responseMs:performance.now()-c.sequenceRoundStarted});
        const ending=c.sequenceConsecutiveMistakes>=3;
        const fb=$('#sequenceFeedback');
        fb.textContent=ending?'Incorrect three times in a row. Moving to the next game.':'Incorrect — that is okay. Moving to the next round.';
        fb.className='sequence-feedback incorrect';
        setMood('encourage','encouraging');
        setStatus(ending?'Three misses in a row — moving on.':'Incorrect. Next round.');
        if(ending){
          c.sequenceLocked=true;
    ...
    function completeSequenceGame(){
      ...
      const avgResponse=(c.sequenceRoundResults||[]).reduce((sum,r)=>sum+Number(r.responseMs||0),0)/Math.max(1,c.sequenceRoundResults?.length||1);
      c.correct=score>=50?1:0;
      c.score=score;
      c.seconds=(performance.now()-c.started)/1000;
      c.sequenceAverageResponseMs=Math.round(avgResponse);
