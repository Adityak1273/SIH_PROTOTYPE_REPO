/* Mino voice-mode control: Continuous or Manual. */
(() => {
  'use strict';
  ...
  function refreshButtons(){
    ...
    if(mic){const title=mode==='manual'?'Tap to talk to Mino':'Talk to Mino';if(mic.title!==title)mic.title=title;if(mic.getAttribute('aria-label')!==title)mic.setAttribute('aria-label',title)}
  }
  function updateVoicePrompt(){const p=document.querySelector('#l3VoiceBox p');if(!p)return;const text=getMode()==='manual'?'Manual mode: tap the microphone each time you want to speak to Mino.':'Continuous mode: after Mino replies, he listens again automatically.';if(p.textContent!==text)p.textContent=text}
  ...
  function ensureSettingsControl(){
    ...
    const control=document.createElement('div');control.className='ccner-voice-mode-control';control.setAttribute('aria-label','Mino voice mode');
