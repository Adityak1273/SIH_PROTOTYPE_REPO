...
async function flush(){if(!navigator.onLine){renderSync();return}const q=read(K.outbox,[]),pending=q.filter(x=>x.status==='pending');if(!pending.length){renderSync();return}window.dispatchEvent(new CustomEvent('ccner:secure-outbox-request',{detail:{ids:pending.map(x=>x.id)}}));renderSync()}
