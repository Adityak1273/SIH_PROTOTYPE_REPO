...
(function(){
  const rounds=[
    {category:'Kitchen',items:['Steel plate','Cooking spoon','Masala box','Tea cup']},
    {category:'Bathroom',items:['Soap','Toothbrush','Bath towel','Comb']},
    {category:'Study Table',items:['Notebook','Pen','Reading glasses','Calendar']},
    {category:'Bedroom',items:['Pillow','Bedsheet','Shawl','Alarm clock']},
    {category:'Living Room',items:['Sofa cushion','Newspaper','Remote control','Table lamp']},
    {category:'Dining Area',items:['Dinner plate','Serving bowl','Water glass','Table mat']},
    {category:'Cleaning',items:['Broom','Dustpan','Cleaning cloth','Mop']},
    {category:'Medicine Routine',items:['Medicine box','Pill organizer','Water glass','Reminder card']},
    {category:'Morning Routine',items:['Toothbrush','Face towel','Comb','Tea cup']},
    {category:'Cooking',items:['Cooking pot','Spatula','Ladle','Cutting board']},
    {category:'Reading',items:['Story book','Newspaper','Reading glasses','Bookmark']},
    {category:'Clothing',items:['Shirt','Socks','Shawl','Clothes hanger']},
    {category:'Grocery Shopping',items:['Shopping bag','Rice packet','Vegetable basket','Milk packet']},
    {category:'Garden',items:['Plant pot','Watering can','Garden gloves','Small trowel']},
    {category:'Going Outside',items:['Walking shoes','Umbrella','Keys','Small bag']}
  ];
  ...
  function install(){
    ...
    function runCategoryRound(){
      ...
      const distractorCount=c.categoryRound<5?3:4;
      const distractors=shuffle(categories.filter(x=>x!==base.category)).slice(0,distractorCount);
      const choices=shuffle([base.category,...distractors]);
      c.categoryExpected=base.category;c.categoryItems=shuffle([...base.items]);c.categoryLocked=false;
      ...
      $('#gameArea').innerHTML=`<div class="category-stage category-transition">
        <div class="category-meta"><span class="category-chip">Question <strong>${c.categoryRound} / ${c.categoryMaxRound}</strong></span><span class="category-chip">Score <strong>${c.categoryCorrect}</strong></span><span class="category-chip">Mistakes <strong>${c.categoryConsecutiveMistakes} / 3</strong></span></div>
        <div class="category-progress-line"><span style="width:${(c.categoryRound-1)/c.categoryMaxRound*100}%"></span></div>
        <div class="category-instruction">Which group do these things belong to?</div>
        <div class="category-items">${c.categoryItems.map((item,i)=>`<div class="category-item" style="--delay:${i*55}ms">${item}</div>`).join('')}</div>
        ...
        <div class="category-progress" id="categoryProgress">${c.categoryRound} of ${c.categoryMaxRound}</div>
      </div>`;
      say('Look at the items carefully. Which group do they belong to?','happy','curious');
    ...
    function answerCategory(answer){
      ...
      if(ok){
        ...
        const fb=$('#categoryFeedback');fb.textContent=c.categoryConsecutiveMistakes>=3?'Three misses in a row — that is okay. Moving to the next game.':'Not quite. That is okay — moving to the next one.';fb.className='category-feedback incorrect';
    ...
    function completeCategory(){
      ...
      state.results.push({...c,name:catalog.find.name,skipped:false,rounds:c.categoryTotal,correctItems:c.categoryCorrect,totalItems:total,consecutiveMistakes:c.categoryConsecutiveMistakes||0});
