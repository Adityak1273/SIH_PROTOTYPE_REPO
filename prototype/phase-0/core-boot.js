...
(function(){
  ...
  function start(){if(typeof window.startSession==='function')return window.startSession();const status=$('#todayStatus');if(status)status.textContent='App is still loading…';}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{loadPhase6();loadPhase7();},{once:true});else{loadPhase6();loadPhase7();}
})();
