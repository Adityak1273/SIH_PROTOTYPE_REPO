/* Mimo Live Voice — Gemini Live API adapter.
...
async function start(){if(state.running||state.connecting)return;state.connecting=true;status('Connecting Mimo voice',true);setMood('thinking','thinking');try{if(!navigator.mediaDevices?.getUserMedia||!window.WebSocket)throw Error('Voice input is unavailable');const t=await token();state.outputCtx=new (window.AudioContext||window.webkitAudioContext)({sampleRate:24000});await state.outputCtx.resume();await openSocket(t);await startMic();state.running=true;state.connecting=false;status('Mimo is listening',true);speech('I’m listening. You can talk naturally.');setMood('listening','listening')}catch(e){state.connecting=false;stopMicOnly();try{state.ws?.close()}catch{}state.ws=null;state.running=false;console.warn('[Mimo Live] fallback:',e?.message||e);status('Voice ready');window.armVoice?.__ccnerOriginal?.();}}
