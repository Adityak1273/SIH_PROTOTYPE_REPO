...
(function(){
  ...
  function start(){if(window.CCNERGameRestored?.startSession)return window.CCNERGameRestored.startSession();if(typeof window.startSession==='function')return window.startSession();const status=$('#todayStatus');if(status)status.textContent='App is still loading…';}
