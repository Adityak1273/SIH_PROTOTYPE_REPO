...
(function(){
  ...
  function nextDate(time){const now=new Date();const [h,m]=String(time||'').split(':').map(Number);if(!Number.isFinite(h)||!Number.isFinite(m))return null;const d=new Date(now);d.setHours(h,m,0,0);if(d<=now)d.setDate(d.getDate()+1);return d}
  async function scheduleOne(r){const p=native();if(!p||r.enabled===false||r.active===false)return false;const at=nextDate(r.time||r.reminder_time);if(!at)return false;const id=stableId(r.id||r.client_id||r.title);try{await p.cancel({notifications:[{id}]});await p.schedule({notifications:[{id,title:r.title||'Reminder',body:kindText(r),channelId:CHANNEL_ID,schedule:(r.repeat||r.repeat_rule)==='daily'?{at,repeats:true,every:'day',allowWhileIdle:true}:{at,allowWhileIdle:true},extra:{reminderId:r.id||null,kind:r.kind||'general'}}]});return true}catch(_){return false}}
